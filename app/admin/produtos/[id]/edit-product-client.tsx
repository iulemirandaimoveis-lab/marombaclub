...
import {
  ...
  DollarSign, Tag, Image as ImageIcon, Star, Barcode, Weight,
  Sparkles, Trash2, Upload, X, Plus,
} from "lucide-react";
...
export function EditProductClient({ product, categories }: { product: any; categories: any[] }) {
  ...
  const [flavors, setFlavors] = useState<string[]>(product.flavors ?? (product.flavor ? [product.flavor] : []));
  const [sizes, setSizes] = useState<string[]>(product.sizes ?? (product.weight_volume ? [product.weight_volume] : []));
  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      ...
      await updateProduct(product.id, {
        ...
        flavor: flavors[0] ?? values.weight_volume?.trim() ?? null,
      ...
      <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }}
        ...
        <div className="flex gap-3">
          <Button variant="outline" asChild>
            <a href={`/admin/produtos/${product.id}`}><Package className="w-4 h-4" />Continuar editando</a>
          </Button>
    ...
    <div className="space-y-8 max-w-3xl">
      <motion.div initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}
        ...
        <Button variant="ghost" size="sm" onClick={handleDelete} className="text-danger hover:text-danger hover:bg-danger/10">
          <Trash2 className="w-4 h-4" /><span className="hidden sm:inline">Desativar</span>
        </Button>
      </motion.div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <Card className="border-border">
          ...
          <CardContent className="space-y-5">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              ...
              <div>
                <label className="block text-xs font-semibold text-muted uppercase tracking-wider mb-2">Slug (URL)</label>
                <Input {...register("slug")} placeholder="whey-protein-isolate"
            ...
            <div>
              <label className="block text-xs font-semibold text-muted uppercase tracking-wider mb-2">
                <Weight className="w-3 h-3 inline mr-1 text-muted" />Tamanhos / Gramagens disponíveis
              </label>
              <div className="flex gap-2 mb-2 flex-wrap">
                {sizes.map((s) => (
            ...
            {categories.length > 0 && (
              <div>
                <label className="block text-xs font-semibold text-muted uppercase tracking-wider mb-2">Categoria</label>
        ...
        <div className="flex items-center gap-3 pb-8">
          <Button type="submit" disabled={submitState === "loading" || uploading} size="lg" className="min-w-[180px]">
            {submitState === "loading" ? <><Loader2 className="w-4 h-4 animate-spin" />Salvando…</> : <><Package className="w-4 h-4" />Salvar alterações</>}
          </Button>
