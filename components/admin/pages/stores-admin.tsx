...
import {
  Store, Plus, Edit, Phone, MapPin, CheckCircle, X,
  Loader2, Package, Truck, Mail,
  Navigation, ToggleLeft, ToggleRight, Building,
} from "lucide-react";

import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { createStore, updateStore, toggleStoreActive } from "@/app/actions/stores";
...
export function StoresAdmin({ stores }: { stores: Store[] }) {
  ...
  function openEdit(store: Store) {
    ...
    setForm({
      ...
      base_delivery_fee_cents: store.base_delivery_fee_cents
        ? store.base_delivery_fee_cents / 100
        : 0,
  ...
  function handleSave() {
    ...
    startTransition(async () => {
      try {
        if (editStore) {
          await updateStore(editStore.id, input);
        } else {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          { label: "Abertas", value: stores.filter(s => s.status === "open" || (s.is_active && !s.status)).length, color: "text-primary" },
          ...
          >
            ...
            <div className="p-5 space-y-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <Field label="Raio de entrega (km)">
                    <Input
                      ...
                      onChange={e =>
                        setForm(f => ({ ...f, delivery_radius_km: parseFloat(e.target.value) || 0 }))
                      }
                    />
                  </Field>
                  <Field label="Taxa de entrega base (R$)">
                    <Input
                      ...
                      step="0.01"
                      placeholder="0.00"
                      value={form.base_delivery_fee_cents}
                      onChange={e =>
                        setForm(f => ({
                          ...f,
                          base_delivery_fee_cents: parseFloat(e.target.value) || 0,
                        }))
                      }
                    />
                  </Field>
                  <Field label="Pedido mínimo (R$)">
                    <Input
                      ...
                      onChange={e =>
                        setForm(f => ({ ...f, min_order_cents: parseFloat(e.target.value) || 0 }))
                      }
                    />
                  </Field>
                </div>
              ...
              <div className="flex gap-3">
                <Button onClick={handleSave} disabled={isPending || formSuccess} size="md">
                  {isPending ? (
                    <>
                    ...
                    <>
                      <Plus className="w-4 h-4" />
                      {editStore ? "Salvar alterações" : "Criar loja"}
                    </>
                  )}
                </Button>
                <Button variant="ghost" size="md" onClick={() => setShowForm(false)} disabled={isPending}>
                  Cancelar
                </Button>
              </div>
            </div>
              ...
              <div className="p-5">
                ...
                <div className="flex items-center gap-2 mt-3 pt-3 border-t border-border/60 flex-wrap">
                  ...
                  {store.delivery_radius_km && (
                    <span className="text-xs text-muted">{store.delivery_radius_km}km</span>
                  )}
                </div>
